  ...
  .deepPopulate('user answers.question')
    ...
    .then(node => {
      ...
      answers.forEach(answer => {
        node.answers
          .filter(nodeAnswer => nodeAnswer.answers.indexOf(answer) > -1)[0]
          .question.answers.filter(nodeAnswer => nodeAnswer._id == answer)
          .forEach(itemAnswer => result += itemAnswer.socionicType);
      });
